...
The **`problem-solving`** skill evaluates approaches in one pass. **This skill adds mandatory upfront framing, then exactly ten interactive questions (one per user turn), then the full evaluation.** Do not skip Phase 2. Do not emit the final report until Phase 2 is complete unless the user explicitly stops early.
